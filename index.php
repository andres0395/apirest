        ...
        <div class="container-fluid p-0">
            <div class="bg-primary d-flex" id="header_tabs">
                ...
                <div class="border-0 col-auto nav nav-tabs p-0" id="nav-tab2" role="tablist">
                    <button class="d-none btn_tab fw-bold nav-link text-white px-1 px-sm-3 " id="tab_2-tab" data-bs-toggle="tab" data-bs-target="#tab_2" type="button" role="tab" aria-controls="tab_cons_mat" aria-selected="true">Inventario</button>
                </div>
            </div>
        </div>
                        ...
                        <main class="col h-100 columns position-relative px-4">
                            ...
                            <div class="d-flex justify-content-center">
                                <div class="mr-5">
                                    ...
                                    <div id="nav_side" class="d-none bg-primary ml-n2 navigation h-100">
                    ...
                    success: function(data){
                        ...
                        if(!!workData.dataTecnico.length){
                            workData.dataTecnico.reduce((acc,e)=>{
                                ...
                                let fil = workData.dataCate.filter(a=>a.parametros.permisos.admin.includes(cedula_sesion) && a.categoria.toUpperCase() == e.categoria.toUpperCase());
                                console.log(fil);
                                if(!acc.includes(e.categoria) && !!fil.length){
                                    ...
                                    const cate = formatBaseMinus(e.categoria);
                                    ...
                                    button.setAttribute('data-target',`#div_cate${e.categoria.replaceAll(' ','')}`);
                                    button.setAttribute('aria-expanded','true');
                                    button.setAttribute('aria-controls',`div_cate${e.categoria.replaceAll(' ','')}`);
                                    button.appendChild(div_c);
                                    let h2 = document.createElement('h2');
                                    h2.classList.add('accordion-header');
                                    h2.setAttribute('id',`h2_${e.categoria.replaceAll(' ','')}`);
                                    ...
                                    table.setAttribute('id',`h_${e.categoria.replaceAll(' ','')}`);
                                    ...
                                    div_div.setAttribute('id',`div_cate${e.categoria.replaceAll(' ','')}`);
                                    div_div.setAttribute('data-parent','#acor_cate');
                                    div_div.setAttribute('aria-labelledby',`h2_${e.categoria.replaceAll(' ','')}`);
                        ...
                        if(!!dataSal.length){
                            dataSal.reduce((acc,e)=>{
                                ...
                                if(!acc.includes(e.categoria) && !!fil.length){
                                    ...
                                    // const cate1 = formatBaseMinus(e.categoria);
                                    let div_super = document.createElement('div');
                                    div_super.classList.add('acordeon_cate', 'accordion-item');
                                    let span = document.createElement('span');
                                    span.classList.add('badge', 'badge-primary', 'border');
                                    ...
                                    let table = document.createElement('table');
                                    table.classList.add('table_kardex', 'table', 'table-bordered', 'table-sm', 'text-center', 'table-hover', 'table-striped');
                                    table.setAttribute('id', `tabla_cate_${e.categoria.replaceAll(' ','')}`);
                                    table.appendChild(thead);
        ...
        function getCate(){
            if(validarSesion(location.pathname+location.search)) {
                pro_ajax = $.ajax({
                    ...
                    dataType : "json",
                    success: function(data){
                        workData.dataCate = data;
                        workData.dataCate.forEach(e=>e.parametros = JSON.parse(e.parametros));
                        let dat = workData.dataCate.reduce((acc,e)=>{
                            acc.push(e.edit);
                            return acc;
                        },[]);
                        console.log(dat);
                        ...
                        let new_btn1 = document.createElement('button');
                            // new_btn1.classList.add('btn_tab', 'fw-bold', 'nav-link', 'text-white', 'px-1', 'px-sm-3','active');
                            // new_btn1.setAttribute('id','tab_1-tab');
                            // new_btn1.setAttribute('data-bs-toggle','tab');
                            // new_btn1.setAttribute('data-bs-target','#tab_1');
                            // new_btn1.setAttribute('type','button');
                            // new_btn1.setAttribute('role','tab');
                            // new_btn1.setAttribute('aria-controls','tab_cons_mat');
                            // new_btn1.setAttribute('aria-selected','true');
                            // new_btn1.textContent = 'Tecnico';
                            // document.getElementById('nav-tab').appendChild(new_btn1);
                        if(!!dat.includes(cedula_sesion)){
                            document.getElementById('tab_2-tab').classList.remove('d-none');
                            // let new_btn = document.createElement('button');
                            // new_btn.classList.add('btn_tab', 'fw-bold', 'nav-link', 'text-white', 'px-1', 'px-sm-3');
                            // new_btn.setAttribute('id','tab_2-tab');
                            // new_btn.setAttribute('data-bs-toggle','tab');
                            // new_btn.setAttribute('data-bs-target','#tab_2');
                            // new_btn.setAttribute('type','button');
                            // new_btn.setAttribute('role','tab');
                            // new_btn.setAttribute('aria-controls','tab_cons_mat');
                            // new_btn.setAttribute('aria-selected','false');
                            // new_btn.textContent = 'Inventario';
                            // document.getElementById('nav-tab2').appendChild(new_btn);
                        }
                    },
                    error: function( jqXHR, textStatus, errorThrown ) {
                        error_ajax( jqXHR, textStatus, errorThrown );
